...
public class AssignmentOne {
    ...
    // Part 3 - Polymorphism
        /*
        showing polymorphism here. Polymorphism is seen with inheritance -- you have a superclass and potentially many child
        classes. Inheritance lets attributes get passed down, and polymorphism allows us to change/modify these passed down
        attributes in the child classes. This is seen with the demonstratePolymorphism() method. The method demonstratePolymorphism()
        can pass any product to it due to the parameter (and since every product has a calculatePrice() method, we see when
        this method is called in main that we get different outcomes. The different outcomes is due to the child classes
    ...
    // Part 5 - Generic methods
        //Makes the ArrayList stop sorting from its natural sorting style and therefore allows us to sort our ArrayList from our compareTo method in Paperback.
        paperbackArray.sort(null);
        //sorts (using compareTo method in Paperback) and prints out the ArrayList called paperbackArray. Prints it out.
    ...
    // Part 6 - Wildcards
        //calling the method called demonstrateWildCards(), and getting the value from my ArrayList called paperbackArray. (Pretty sure I have done this section wrong.)
    ...
    //Part 7 - Simple lambda expressions
        /*
        Calling the method called DemonstrateLambdas()  and checking what is true or false regarding our ArrayList called PaperbackArray.
    ...
    // Method for Part 6 - Wildcards
        /*
        Have an ArrayList generic as the parameter (question mark represents an unknown value type). Using a wildcard
         with an upper bound (extends) to get any list type of Object (PhyicalProduct class). We then return objects value at index 3
         (method within PhyicalProduct called calculatePrice. Should therefor return a price with 10% added to it.)
         */
    ...
    // Method for Part 7 - lambdas
        //Lambda expressions are chunks of code that can be randomly placed in the body of a method
        //Getting what books are unavailable (boolean) and prints out saying what these objects (names only) are.
        //Note: This method takes 2 things Predicate (interface) and ArrayList (called books) with type Paperback. (Method uses generics -- type.)
